...
(function () {
  ...
  function retryLastAction() {
    ...
    if (last) {
      if (last.kind === 'answer') {
        ...
        return;
      } else if (last.kind === 'describe') {
        appendMessage({ kind: 'system', text: COPY.GENERATING_MSG });
        setInFlight(true);
        CTX.describe(last.prose, last.gloss)
          .catch(function (err) {
            // Re-use surfaceFinalError logic but avoid importing it directly if it's messy
            // Actually handleError exists in this file, let's look at how correct() handles it:
            // .catch(handleError)
            handleError(err);
          })
          .then(function () { setInFlight(false); });
  ...
  function maybeSurfaceGenerationError(snap) {
    ...
    if (CTX && typeof CTX.forceGenerate === 'function' && !chat.autoRetriedGen && snap.parametersPartial) {
      chat.autoRetriedGen = true;

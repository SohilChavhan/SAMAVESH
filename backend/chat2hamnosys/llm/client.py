...
def _resolve_api_key(explicit: str | None) -> str:
    """Pick the API key to use: explicit > request contextvar > env (GROQ > OPENAI > GEMINI)."""
...
class LLMClient:
    ...
    def __init__(
        ...
        *,
        ...
        is_groq = resolved.startswith("gsk_")
        if is_groq:
            self.model = model if model is not None else "llama-3.1-8b-instant"
            self.fallback_model = (
                fallback_model if fallback_model is not None else "llama-3.1-8b-instant"
            )

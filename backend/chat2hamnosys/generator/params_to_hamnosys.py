...
def _compose_pieces(params: PartialSignParameters) -> list[_Piece]:
    ...
    handshape = params.handshape_dominant or "flat"
    chunk, ok = _vocab_resolve("handshape", handshape)
    if not ok:
        chunk, ok = _vocab_resolve("handshape", "flat")
    pieces.append(
        _Piece(
    ...
    ext_finger = params.orientation_extended_finger or "up"
    chunk, ok = _vocab_resolve("orientation_ext_finger", ext_finger)
    if not ok:
        chunk, ok = _vocab_resolve("orientation_ext_finger", "up")
    pieces.append(
        _Piece(
    ...
    palm = params.orientation_palm or "towards signer"
    chunk, ok = _vocab_resolve("orientation_palm", palm)
    if not ok:
        chunk, ok = _vocab_resolve("orientation_palm", "towards signer")
    pieces.append(
        _Piece(
